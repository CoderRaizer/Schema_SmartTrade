...
BEGIN
	...
	SELECT TEMP.MACP AS MACP,
	  ...
	  -- ADD Data Khop Lenh (Neu co)
	  DECLARE @GIAKHOP FLOAT = (SELECT TOP (1) LK.GIAKHOP FROM dbo.LENHKHOP AS LK WHERE LK.IDLENHDAT
	  IN (SELECT LD.ID FROM dbo.LENHDAT AS LD WHERE LD.MACP = @MACP) ORDER BY LK.NGAYKHOP DESC)

	  DECLARE @KHOILUONGKHOP INT = (SELECT TOP (1) LK.SOLUONGKHOP FROM dbo.LENHKHOP AS LK WHERE LK.IDLENHDAT
	  IN (SELECT LD.ID FROM dbo.LENHDAT AS LD WHERE LD.MACP = @MACP) ORDER BY LK.NGAYKHOP DESC)

	  UPDATE dbo.BANGGIATRUCTUYEN SET GIAKHOP = @GIAKHOP, KHOILUONGKHOP = @KHOILUONGKHOP WHERE MACP = @MACP
...
BEGIN
	...
	-- << Begin
		...
		SELECT @GIA_MUA1 = BGTT.GIAMUA1 FROM dbo.BANGGIATRUCTUYEN AS BGTT WHERE BGTT.MACP = @MACP
		SELECT @GIA_MUA2 = BGTT.GIAMUA2 FROM dbo.BANGGIATRUCTUYEN AS BGTT WHERE BGTT.MACP = @MACP

		-- IF GiaMua1 Is Not Null
		IF (@GIA_MUA1 IS NOT NULL)
		BEGIN
			-- equal GiaMua1
			IF (@GIADAT = @GIA_MUA1)
			BEGIN
			...
			BEGIN
				...
				UPDATE dbo.BANGGIATRUCTUYEN
				SET GIAMUA2 = @GIA_MUA1, KLM2 = KLM1 WHERE MACP = @MACP
		...
		-- IF GiaMua2 Is Not Null
		IF (@GIA_MUA2 IS NOT NULL)
		BEGIN
			-- equal GiaMua2
			IF (@GIADAT = @GIA_MUA2)
			BEGIN
	...
	-- << Begin
		...
		SELECT @GIA_BAN1 = BGTT.GIABAN1 FROM dbo.BANGGIATRUCTUYEN AS BGTT WHERE BGTT.MACP = @MACP
		SELECT @GIA_BAN2 = BGTT.GIABAN2 FROM dbo.BANGGIATRUCTUYEN AS BGTT WHERE BGTT.MACP = @MACP

		-- IF GiaBan1 Is Not Null
		IF (@GIA_BAN1 IS NOT NULL)
		BEGIN
			-- equal GiaBan1
			IF (@GIADAT = @GIA_BAN1)
			BEGIN
			...
			BEGIN
				UPDATE dbo.BANGGIATRUCTUYEN
				SET GIABAN2 = @GIA_BAN1, KLB2 = KLB1 WHERE MACP = @MACP
		...
		-- IF GiaBan2 Is Not Null
		IF (@GIA_BAN2 IS NOT NULL)
		BEGIN
			-- equal GiaBan2
			IF (@GIADAT = @GIA_BAN2)
			BEGIN
...
AS
	BEGIN --  Begin Trigger
		...
		SET @LoaiGD = (SELECT LD.LOAIGD FROM dbo.LENHDAT AS LD WHERE LD.ID = @IDLENHDAT)

		UPDATE dbo.BANGGIATRUCTUYEN
		SET GIAKHOP = @GIAKHOP, KHOILUONGKHOP = @SOLUONGKHOP
		WHERE MACP = @MACP

		-- <<Start Check Value in BGTT>> --
		----------------------------------------------------------------------------------------
		SELECT @GIA_MUA1 = BGTT.GIAMUA1 FROM dbo.BANGGIATRUCTUYEN AS BGTT WHERE BGTT.MACP = @MACP
		SELECT @KLM1 = BGTT.KLM1 FROM dbo.BANGGIATRUCTUYEN AS BGTT WHERE BGTT.MACP = @MACP
		SELECT @GIA_MUA2 = BGTT.GIAMUA2 FROM dbo.BANGGIATRUCTUYEN AS BGTT WHERE BGTT.MACP = @MACP
		SELECT @KLM2 = BGTT.KLM2 FROM dbo.BANGGIATRUCTUYEN AS BGTT WHERE BGTT.MACP = @MACP
		----------------------------------------------------------------------------------------

		-- Case GiaKhop = GiaMua1
		IF (@GIAKHOP = @GIA_MUA1)
		BEGIN
			-- Neu SoLuongKhop nho hon KLM1 dang co
			IF (@SOLUONGKHOP < @KLM1)
			BEGIN
				UPDATE dbo.BANGGIATRUCTUYEN
				SET KLM1 = @KLM1 - @SOLUONGKHOP
				WHERE MACP = @MACP
				RETURN
			END

			-- Neu SoLuongKhop bang voi KLM1 dang co
			IF (@SOLUONGKHOP = @KLM1)
			BEGIN
				UPDATE dbo.BANGGIATRUCTUYEN
				SET GIAMUA1 = NULL, KLM1 = NULL
				WHERE MACP = @MACP
				RETURN
			END
		END

		-- Case GiaKhop = GiaMua2
		IF(@GIAKHOP = @GIA_MUA2)
		BEGIN
			-- Neu SoLuongKhop nho hon KLM2 dang co
			IF (@SOLUONGKHOP < @KLM2)
			BEGIN
				UPDATE dbo.BANGGIATRUCTUYEN
				SET KLM2 = @KLM2 - @SOLUONGKHOP
				WHERE MACP = @MACP
				RETURN
			END
