	  ...
	  FROM 
	   (
		   ...
		   FROM (
				...
				WHERE LD.LOAIGD LIKE 'M' AND LD.MACP LIKE 'PTIT' AND LD.SOLUONG > 0
				ORDER BY LD.GIADAT DESC
				UNION
				SELECT TOP 2 *, RN = ROW_NUMBER() OVER (PARTITION BY MACP, LOAIGD ORDER BY GIADAT ASC)
				FROM dbo.LENHDAT AS LD
				WHERE LD.LOAIGD LIKE 'B' AND LD.MACP LIKE 'PTIT' AND LD.SOLUONG > 0
...
EXEC SP_FILL_NEW_DATA_INTO_BANGGIATRUCTUYEN
